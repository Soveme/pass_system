    ...
    <table class="users-table">
      ...
      <tbody>
        <tr v-for="user in users" :key="user.id">
          ...
          <td>
            <button @click="toggleUserStatus(user)" :class="['btn-toggle', user.is_active ? 'active' : 'inactive']">
...
const loadUsers = async () => {
  try {
    ...
    // Filter only users, not passes
    users.value = response.data.filter(item => item.username !== undefined && item.role !== undefined)
...
const toggleUserStatus = async (user) => {
  try {
    ...
    const response = await apiClient.patch(`/api/admin/users/${user.id}/status`, 
